...
removeURL <- function(x) gsub("http^\\s\\s*", "", x)%>% 
  clean_corpus <- tm_map(text_corpus, content_transformer(removeURL))
...
five_words <- findFreqTerms(email_dtm_train, 5)
length(five_words) #how many words are there?
five_words[1:5]

#Create DTMs using frequent words:
email_train <- DocumentTermMatrix(email_corpus_train, control=list(dictionary = five_words))
email_test <- DocumentTermMatrix(email_corpus_test, control=list(dictionary = five_words))
...
Our (randomized) input data was 35.83% spam and 64.17% ham.

As you can see above, of our 779 test documents (emails), 13.00% (65/500) were accurately labeled "ham" (the ham-ham, 1st row, 1st column entry) and 96.77% (270/279) were accurately labeled "spam" (the spam-spam, 2nd row, 2nd column entry).
