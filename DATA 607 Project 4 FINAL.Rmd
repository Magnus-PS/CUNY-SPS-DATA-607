...
set.seed(1228)
sf_final<- df_final[sample(nrow(df_final)),]
...
```
We can note the prominence of words like "received" and "localhost" and jumbled words like "esmtp" and "dogmaslashnullorg" ... at this point we're ready to enter the final phase. We're ready to ...
...
##do we do anything with the values of spam and ham???
...
It does not appear that our model performed well and its output was not quite in the form we were expecting. We were hoping to get a breakdown of spam and ham as outputs from our prediction but we ended up with just predictions for ham and it seems these predictions were highly inaccurate ... 

Our input was randomized at a proportion of 35.83% spam, 64.17% ham ...

Even still, our prediction of ham messages had a measly accuracy of 24% (189/779), having mis-classifed spam ~76% of the time.

We'd encountered this error after first implementing naive Bayes and tried randomizing our inputs (df_final), varying sparsity (alternating between dtm and dtm_filtered), as well as varying the order of our corpus cleaning functions. Ultimately none of these alterations improved our accuracy.

What's left, revisit the naive Bayes model itself. We'd followed the flow of our first reference (below) and it seems this exact method did not work for us and thus there must be some slight alteration needed. Possibly in the convert_count() function or in increasing the amount of appearances necessary to make it into our frequency dictionary ...
